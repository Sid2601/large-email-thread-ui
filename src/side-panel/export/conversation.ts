...
const MASK_NOTICE = 'Identities are masked. Every name, email address, company domain, phone number, link address and message id was replaced by a consistent placeholder such as Person 1 or person1@company1.example, and no images are included. Message wording, formatting, order, timestamps and recovery labels are unchanged, so the same copy still reproduces a parsing problem. The placeholder-to-person map is not written into this file.';

/** A portable, script-free document; never export expiring attachment URLs. */
export function conversationHtml(thread: ThreadData, exportedAt = new Date(), options: { masked?: boolean } = {}): string {
 ...
 * `mask: true` writes that same small copy with every identity replaced, for sharing a thread for diagnosis.
 */
export async function downloadConversation(thread: ThreadData, options: { includeImages?: boolean; mask?: boolean } = {}): Promise<number> {
  const date = new Date();
  // A masked copy never embeds image bytes: a picture can show a face, a signature or a letterhead.
  const includeImages = options.mask ? false : options.includeImages !== false;
