...
### Inline image download added to the pending 1.7.4 release

The full-size image viewer now places **Download image** on the same row as **Close image** in both production and Dev. It downloads only the selected image and does not restore whole-conversation exports to production. The action reuses `loadInlineImage`, including its authenticated fetch, source-tab blob resolution, bounded cache/concurrency and existing 8 MB raster limit. Already-resolved data images need no network request. The downloaded Blob holds the original decoded bytes: there is no canvas conversion, resizing or screenshot, so formats such as GIF remain unchanged. Filenames use an available image filename or caption, sanitize filesystem-special characters, and derive the extension from the actual downloaded MIME type.

Download state disables duplicate clicks, shows Downloading while work is pending and leaves Close image usable. A failure is shown inside the dialog and enables retry. The viewer uses a flex layout so its image area can still scroll beneath the action row or an error. Image loading/reconciliation and attachment behavior are unchanged; no new permissions or automatic downloads are added.

`tests/image-download.ts` adds eight cases for naming, MIME/extension agreement, exact bytes, forwarding source-tab identity and failure handling. Package smoke checks the real prod/dev image viewer with HTTPS, Gmail blob and embedded data images, compares every saved file with its original bytes, checks both buttons share a row and the viewer remains open, and exercises a displayed image whose subsequent retrieval is refused. Both package contracts now require the Download image control. PR #23 remains the pending release review, so these changes are included in 1.7.4 rather than starting a second release before it is merged.

**Image download verification completed:** both updated 1.7.4 package pipelines pass typecheck and 251 tests (one optional capture-file test skipped). Both packaged extensions pass Chromium checks for HTTPS/blob/data image downloads, exact saved bytes, same-row controls, failure/retry UI, retained attachment flows, and dev-only conversation exports. There are no page errors. The production viewer screenshot is at ignored `artifacts/packages-1.7.4/prod-image-viewer.png`.
